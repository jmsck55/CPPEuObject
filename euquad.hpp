...
// euquad.hpp
//
// quadmath.h is a GCC header library.
...
{
        extern "C" {
                #include "be_quad.h" // See: "euquad.hpp"
        }

// perhaps, quadmath.h is not optimized.
// What about gmp.h and mpfr.h ???

        class Quadmath : public base_class
        {
        private:
        ...
        public:
                ...
                ~Quadmath() { DeRefObj(); obj = NOVALUE; } // default destructor
                Quadmath(const Quadmath& x) { obj = x.obj; RefObj(); } // copy constructor
                //Quadmath& operator= (const Quadmath& x) { DeRefObj(); obj = x.obj; RefObj(); return *this; } // copy assignment
                //Quadmath (Quadmath&& x) { obj = x.obj; x.obj = NOVALUE; } // move constructor
                //Quadmath& operator= (Quadmath&& x) { DeRefObj(); obj = x.obj; x.obj = NOVALUE; return *this; } // move assignment
		
                //Quadmath(d_ptr ptr) { ++(ptr->ref); obj = MAKE_DBL(ptr); }
                Quadmath(equadmath q) { obj = NewFloat128(q); }
                void NewQuadmath(equadmath d) { DeRefObj(); obj = NewFloat128(d); }
                equadmath GetQuadmath() { return GetFloat128(obj); }
